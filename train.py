...
def save_loss_plot(train_losses, val_losses, val_interval, filename):
    """ Save and update the training and validation loss plot in real-time """
    plt.figure()
...
def main():
    ...
    # sample_input = (torch.randn(config.batch_size, 3, 352, 352).cuda(),
    #                 torch.randn(config.batch_size, 3, 352, 352).cuda())
        ...
        while True:
            for data_x, data_y in data_loader:
                ...
                if config.mix:
                    ...
                    with autocast_fn():
                        ...
                        if model.__class__.__name__ == 'CLIPDensePredTMasked':
                            visual_s_cond, _, _ = model.visual_forward(data_x[2].cuda(), data_x[3].cuda())
                        else:
                    ...
                    if model.__class__.__name__ == 'CLIPDensePredTMasked':
                        with autocast_fn():
                            ...
                            cond, _, _ = model.visual_forward(data_x[1].cuda(), data_x[2].cuda())
                    else:
                        cond = data_x[1]
                ...
                with autocast_fn():
                    ...
                    # loss1 = loss_fn(pred, data_y[0].cuda())
                    loss = Dice_loss(pred, data_y[0].cuda())
                    # + BCE_loss(pred, data_y[0].cuda())
