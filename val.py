...
def main():
    ...
    if config.test_dataset == 'phrasecut':
        ...
        dataset = PhraseCut('val', image_size=352, mask=config.mask, with_visual=config.with_visual,
        ...
        message = """The prediction results with %s: mean_iou: %.3f | mean_dice: %.3f | foreground_iou: %.3f""" \
                  % (train_checkpoint_id, mean_iou, mean_dice, foreground_iou)
